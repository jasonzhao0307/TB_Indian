...
require(boot)
require(gmodels)
sourceDirectory("~/Google Drive File Stream/My Drive/projects/R_lib_jason/", modifiedOnly=TRUE)
```
...
maertzdorf_4gene <- c("GBP1", "ID3", "P2RY14", "IFITM3")
sambarey_10gene <- c("FCGR1A", "HK3", "RAB13", "RBBP8", "IFI44L", "TIMM10", "BCL6", "SMARCD3", "CYP4F3", "SLPI")
...
maertzdorf_4gene_vec <- c()
sambarey_10gene_vec <- c()
...
maertzdorf_4gene_list <- c()
sambarey_10gene_list <- c()
...
for (i in 1:n) {
  ...
  maertzdorf_4gene_list[[i]] <- intersect(DEG_tmp, maertzdorf_4gene)
  sambarey_10gene_list[[i]] <- intersect(DEG_tmp, sambarey_10gene)
  ...
  maertzdorf_4gene_vec <- c(maertzdorf_4gene_vec, length(intersect(DEG_tmp, maertzdorf_4gene))/4)
  sambarey_10gene_vec <- c(sambarey_10gene_vec, length(intersect(DEG_tmp, sambarey_10gene))/10)
}

percentage <- c(jacobsen2007_3gene_vec, sweeney2016_3gene_vec, zak2016_16gene_vec, berry2010_393gene_vec, berry2010_86gene_vec, kaforou_27gene_vec, maertzdorf_4gene_vec, sambarey_10gene_vec)
name <- c(rep("Jacobsen 3gene",n), rep("Sweeney 3genes",n), rep("Zak 16genes",n), rep("Berry 393genes",n), rep("Berry 86genes",n), rep("Kaforou 27genes",n), rep("Maertzdorf 4gene",n), rep("Sambarey 10genes",n))
cutoff <- c(rep(1:n, 8))
...
#### Maertzdorf 4 gene signature
```{r message=FALSE}
ggList <- plot_visualization(IndianData_final[maertzdorf_4gene,], labelIndianData_final, dataName = "Maertzdorf 4 gene", log2Trans = TRUE, colorGroup = c(TB = "red", control = "green"), useLabel = FALSE, pdfOutput = TRUE)
ggList[[1]]
ggList[[2]]
...
plot_pheatmap(IndianData_final[maertzdorf_4gene,], labelIndianData_final,colorGroup = c(TB = "red", control = "green"), title = "Row-scaled Log2-transformed Maertzdorf signature Heatmap", log2Trans = T, rowScaling = T, showColName = T, outputPDF = T, outputName = "Maertzdorf_heatmap.pdf", height = 8, width = 9)

plot_tsne(IndianData_final[maertzdorf_4gene,], labelIndianData_final,title = "Row-scaled Log2-transformed Maertzdorf signature Heatmap", outputPDF = TRUE, outputName = "Maertzdorf_tsne.pdf", height = 14, width = 16)


```


#### Sambarey 10 gene signature
```{r message=FALSE}
ggList <- plot_visualization(IndianData_final[sambarey_10gene,], labelIndianData_final, dataName = "Sambarey 10 gene", log2Trans = TRUE, colorGroup = c(TB = "red", control = "green"), useLabel = FALSE, pdfOutput = TRUE)
ggList[[1]]
ggList[[2]]
...
plot_pheatmap(IndianData_final[sambarey_10gene,], labelIndianData_final,colorGroup = c(TB = "red", control = "green"), title = "Row-scaled Log2-transformed Sambarey signature Heatmap", log2Trans = T, rowScaling = T, showColName = T, outputPDF = T, outputName = "Sambarey_heatmap.pdf", height = 8, width = 9)

plot_tsne(IndianData_final[sambarey_10gene,], labelIndianData_final,title = "Row-scaled Log2-transformed Sambarey signature Heatmap", outputPDF = TRUE, outputName = "Sambarey_tsne.pdf", height = 14, width = 16)
...
maertzdorf.fisher.mat <- FisherTestGeneListEnrichment(maertzdorf_4gene, DEG.1e11, all.gene)
sambarey.fisher.mat <- FisherTestGeneListEnrichment(sambarey_10gene, DEG.1e11, all.gene)
...
CI for AUC
```{r}
dfList <- list(IndianData_final[zak2016_16gene,], IndianData_final[jacobsen2007_3gene,], IndianData_final[sweeney2016_3gene,], IndianData_final[berry2010_393gene_exist_overlap,],IndianData_final[berry2010_86gene_exist,], IndianData_final[signature.lasso.multiple,], IndianData_final[Kaforou.27,], IndianData_final[maertzdorf_4gene,], IndianData_final[sambarey_10gene,])

signatureNameVec <- c("Zak-16", "Jacobsen-3", "Sweeney-3", "Berry-393", "Berry-86", "Indian-lasso", "Kaforou-27", "Maertzdorf-4", "Sambarey-10")
...
for(i in 1:length(dfList)){
  ...
  auc.result.ci[[i]] <- ci(auc.vec.tmp$auc)
...
# update: add sambarey and Maertzdorf signature, 01/03/2018
...
auc.result.order[[1]] <- auc.result[[2]]
auc.result.order[[2]] <- auc.result[[3]]
auc.result.order[[3]] <- auc.result[[8]]
auc.result.order[[4]] <- auc.result[[9]]
auc.result.order[[5]] <- auc.result[[1]]
auc.result.order[[6]] <- auc.result[[5]]
auc.result.order[[7]] <- auc.result[[7]]
auc.result.order[[8]] <- auc.result[[4]]
auc.result.order[[9]] <- auc.result[[6]]
names(auc.result.order) <- c("Jacobsen-3", "Sweeney-3", "Maertzdorf-4", "Sambarey-10", "Zak-16", "Berry-86", "Kaforou-27","Berry-393","Indian-lasso")
...
saveRDS(list(jacobsen = jacobsen2007_3gene, sweeney = sweeney2016_3gene, berry.393 = berry2010_393gene_exist_overlap, berry.86 = berry2010_86gene_exist, kaforou.27 = Kaforou.27), "../tb.signature.rds")
```
